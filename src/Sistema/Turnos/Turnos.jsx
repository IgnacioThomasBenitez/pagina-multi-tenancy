...
import {
  ...
  Calendar, MapPin, Scissors, Sparkles, ChevronRight,
} from 'lucide-react';
...
const STYLES = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .tr-row { transition: background 0.15s ease; cursor: pointer; }
  .tr-row:hover { background: ${t.accent}08 !important; }
  .tr-row.active-row { background: ${t.accent}12 !important; border-left: 2px solid ${t.accent} !important; }
  ...
  /* Animación entrada contenido panel */
  .panel-content {
    opacity: 0;
...
function IconInput({ icon: Icon, type = 'text', value, onChange, placeholder, name }) {
  return (
    <div style={{ position: 'relative' }}>
      <Icon size={14} color={t.textMuted} style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', pointerEvents: 'none' }} />
      <input type={type} name={name} value={value} onChange={onChange} placeholder={placeholder} className="tm2-input" />
    </div>
  );
}
...
/* ─── Modal wrapper (para Nuevo / Editar / Horarios) ─── */
...
export default function TurnosManager() {
  ...
  const updateSchedule = (d, i, f, v) => { const u = [...schedules[d]]; u[i][f] = v; setSchedules({ ...schedules, [d]: u }); };

  /* Turno helpers */
  const addTurno = () => {
    if (!newTurno.hora || !newTurno.cliente) return;
    setTurnos([...turnos, { id: Math.max(...turnos.map(x => x.id)) + 1, ...newTurno, estado: 'Pendiente' }]);
    setNewTurno(EMPTY_TURNO);
    setShowAddModal(false);
  };

  const updateStatus = (id, estado) => {
    setTurnos(prev => prev.map(x => x.id === id ? { ...x, estado } : x));
    // Actualizar turno en el panel sin cerrarlo
    setSelectedTurno(prev => prev ? { ...prev, estado } : prev);
  };

  const deleteTurno = (id) => {
    if (window.confirm('¿Eliminar este turno?')) {
      setTurnos(prev => prev.filter(x => x.id !== id));
      closePanel();
    }
  };
  ...
  const saveEdit = () => {
    ...
    setTurnos(prev => prev.map(x => x.id === editTurno.id ? editTurno : x));
    setSelectedTurno(editTurno);
    setShowEditModal(false);
  };

  const filtered = turnos.filter(x =>
    (selectedLocation === 'Todos' || x.local === selectedLocation) &&
    (selectedStatus   === 'Todos' || x.estado === selectedStatus) &&
    x.cliente.toLowerCase().includes(searchTerm.toLowerCase())
  );

  const stats = [
    { label: 'Total',       val: turnos.length,                                       color: t.info,   Icon: Scissors    },
    { label: 'Finalizados', val: turnos.filter(x => x.estado === 'Finalizado').length, color: t.accent, Icon: CheckCircle },
    { label: 'Pendientes',  val: turnos.filter(x => x.estado === 'Pendiente').length,  color: t.gold,   Icon: AlertCircle },
    { label: 'Cancelados',  val: turnos.filter(x => x.estado === 'Cancelado').length,  color: t.danger, Icon: XCircle     },
  ];

  /* Color del estado del panel */
  const panelStatus = selectedTurno ? (STATUS[selectedTurno.estado] || {}) : {};

  return (
    <>
      <style>{STYLES}</style>

      <div className="tm2-root" style={{ display: 'flex', height: '100vh', background: t.bg, overflow: 'hidden' }}>
        <Sidebar />
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '28px 32px', minWidth: 0 }}>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button className="btn-g" onClick={() => setShowScheduleModal(true)} style={{
              ...
              <button className="btn-p" onClick={() => setShowAddModal(true)} style={{
          ...
          {/* Filtros */}
          <div style={{ background: t.card, border: `1px solid ${t.border}`, borderRadius: 14, padding: '16px 20px', marginBottom: 18 }}>
          ...
          <div style={{ background: t.card, border: `1px solid ${t.border}`, borderRadius: 16, overflow: 'hidden' }}>
            <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <span style={{ fontSize: 11, fontWeight: 700, color: t.textMuted, textTransform: 'uppercase', letterSpacing: '0.6px' }}>Turnos del día</span>
              </div>
              <span style={{ fontSize: 11, color: t.textDim }}>{filtered.length} resultado{filtered.length !== 1 ? 's' : ''}</span>
            </div>
                  ...
                  {filtered.length === 0 ? (
                    <tr><td colSpan={8}>
                      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '48px 20px', gap: 10 }}>
                        <div style={{ width: 52, height: 52, borderRadius: 14, background: t.surface, border: `1px solid ${t.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    ...
                    const isActive = panelOpen && selectedTurno?.id === trn.id;
                    return (
                      <tr
                        key={trn.id}
                        className={`tr-row${isActive ? ' active-row' : ''}`}
                        onClick={() => isActive ? closePanel() : openPanel(trn)}
                      ...
                      >
                        ...
                        <td style={{ padding: '13px 16px' }}>
                          <div className="eye-btn" style={{
                            padding: '6px 8px', borderRadius: 8, color: isActive ? t.accent : t.textMuted,
                            display: 'flex', opacity: isActive ? 1 : undefined,
                          }}>
              ...
              {selectedTurno && (
                <>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    <div>
                      <div style={{ fontSize: 10, color: panelStatus.color, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '1px', marginBottom: 5 }}>
                        Turno #{selectedTurno.id}
                      </div>
                      <div style={{ fontSize: 17, fontWeight: 800, color: t.text, lineHeight: 1.2 }}>
                  ...
                  {/* Status badge grande */}
                  <div style={{
                    background: panelStatus.bg, border: `1px solid ${panelStatus.border}`,
                    borderRadius: 12, padding: '12px 14px',
                    display: 'flex', alignItems: 'center', gap: 8,
                  }}>
                    {panelStatus.Icon && <panelStatus.Icon size={16} color={panelStatus.color} />}
                    <div>
                      <div style={{ fontSize: 10, color: t.textMuted, textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: 2 }}>Estado actual</div>
                      <div style={{ fontSize: 14, fontWeight: 700, color: panelStatus.color }}>{selectedTurno.estado}</div>
                    </div>
                  </div>
                  ...
                  <div style={{ background: t.card, border: `1px solid ${t.border}`, borderRadius: 12, padding: '4px 14px' }}>
                    {[
                      ['Fecha',  selectedTurno.fecha,  <Calendar size={12} color={t.textMuted} />],
                      ['Hora',   selectedTurno.hora,   <Clock    size={12} color={t.textMuted} />],
                      ['Tipo',   selectedTurno.tipo,   <Scissors size={12} color={t.textMuted} />],
                      ['Local',  selectedTurno.local,  <MapPin   size={12} color={t.textMuted} />],
                    ].map(([k, v, icon]) => (
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    <div style={{ fontSize: 10, fontWeight: 700, color: t.textMuted, textTransform: 'uppercase', letterSpacing: '0.6px', marginBottom: 2 }}>Acciones</div>
                ...
                <div key={day} className="day-block">
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: schedules[day].length ? 10 : 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      ...
                      {schedules[day].length === 0 && <span style={{ fontSize: 11, color: t.textDim, fontStyle: 'italic' }}>Sin atención</span>}
                    </div>
                    <button onClick={() => addSchedule(day)} style={{
                  ...
                  {schedules[day].map((s, idx) => (
                    <div key={idx} style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 8 }}>
                      <input type="time" value={s.start} onChange={e => updateSchedule(day, idx, 'start', e.target.value)} style={{ ...inputStyle, flex: 1, padding: '9px 12px' }} />
                      <span style={{ fontSize: 11, color: t.textMuted, flexShrink: 0 }}>hasta</span>
                      <input type="time" value={s.end} onChange={e => updateSchedule(day, idx, 'end', e.target.value)} style={{ ...inputStyle, flex: 1, padding: '9px 12px' }} />
